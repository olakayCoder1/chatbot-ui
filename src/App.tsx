...
// import NEATSupportBot from './NEATSupportBot';
...
//   const handleSubmit = async (e?: SubmitEvent | ButtonEvent): Promise<void> => {
//     e?.preventDefault();
//     if (!e) return;
...
function App(): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement>): Promise<void> => {
    ...
    // Check if it's a button click with suggestion data
    const isButtonClick = e.type === 'click' && 'dataset' in (e.currentTarget as HTMLElement);
    let userQuery = '';
    
    if (isButtonClick) {
      const button = e.currentTarget as HTMLButtonElement;
      userQuery = button.dataset.suggestion || '';
    } else {
      userQuery = input;
    }
